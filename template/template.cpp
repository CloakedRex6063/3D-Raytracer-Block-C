...
{
	app->mouseHidden = !app->mouseHidden;
	glfwSetInputMode( window, GLFW_CURSOR, app->mouseHidden ? GLFW_CURSOR_DISABLED : GLFW_CURSOR_NORMAL );
}
...
void main()
{
	...
	{
		deltaTime = min( 1.0f, 1.f * timer.elapsed() );
		timer.reset();
		app->Tick( deltaTime );
